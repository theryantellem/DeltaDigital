...
"",first_buy=0,qtyusdt=0,first_price=0,balance=0,entry_call=0,stop_loss=0,price_above=0,price_below=0,re_capital=0,closing_price=0,strategy_cal=0,total_profit=0,trade_type=1):
...
def spotBot(user_id, setid, beforeqty, flag, sold, gas, first_buy, first_price, platform, api_key, api_secret, api_passphrase, in_position, buy_position, sell_position, coin, trade_price,
           firstbuy_amount, qty, double_position, margin_call, profit_ratio, whole_ratio, price_drop, m_ratio, profit_callback, buycallback, position_amount):
    while True:
        try:
            trade_type = 0
            market = coin + '/USDT'
            ...
            if in_position == False:
                ...
                if platform == 1 and in_position == False and sell_position == False and buy_position == False and gas > 9 and first_buy == 0:
                    ...
                    in_position = True
                    first_buy = 0
                    link = f"{base_url}/posttrade?setid={setid}&user_id={user_id}&qty={qty}&in_position={in_position}&buy_position={buy_position}&sell_position={sell_position}&trade_price={trade_price}&tgmessage={tgmessage}&first_buy={first_buy}&position_amount={qtyusdt}&first_price={trade_price}&balance={balance}&platform={platform}&trade_type={trade_type}"
                    response = requests.get(link)
            ...
            if in_position == True and buy_position == True and sell_position == False and platform == 1:
                ...
                if trade_price > cal2:
                    ...
                    balance = exchange.fetch_balance()
                    balance = balance['free']['USDT']
                    link = f"{base_url}/settletrade?setid={setid}&user_id={user_id}&qty={qty}&in_position={in_position}&buy_position={buy_position}&sell_position={sell_position}&trade_price={trade_price}&tgmessage={tgmessage}&first_buy={first_buy}&position_amount={qtyusdt}&profit={profit}&coin={coin}&exchange={platform}&balance={balance}&trade_type={trade_type}"
                    response = requests.get(link)
            ...
            if platform == 1 and in_position == True and sell_position == False and buy_position == True and gas > 9 and margin_call > 0 and margin_call != first_buy:
                ...
                if trade_price < cal2:
                    ...
                    buy_position = True
                    in_position = True
                    link = f"{base_url}/posttrade?setid={setid}&user_id={user_id}&qty={qty}&in_position={in_position}&buy_position={buy_position}&sell_position={sell_position}&trade_price={trade_price}&tgmessage={tgmessage}&first_buy={first_buy}&position_amount={qtyusdt}&first_price={first_price}&balance={balance}&platform={platform}&trade_type={trade_type}"
                    response = requests.get(link)
            ...
            link = f"{base_url}/return?setid={setid}&user_id={user_id}&balance={balance}&platform={platform}"
            response = requests.get(link)
            ...
            if "insufficient" in errorMessage:
                ...
                link = f"{base_url}/posttradelog?setid={setid}&user_id={user_id}&tgmessage={tgmessage}&balance={balance}&platform={platform}&trade_type={trade_type}"
                response = requests.get(link)
                link = f"{base_url}/return?setid={setid}&user_id={user_id}&balance={balance}&platform={platform}"
                response = requests.get(link)
...
def featuresBot(user_id, setid, gas, first_buy, first_price, platform, api_key, api_secret, api_passphrase, in_position, buy_position, sell_position, coin, trade_price,
           first_amount, qty, price_drop, m_ratio, position_amount, leverage, direction, stop_loss, price_above, price_below, re_capital, closing_price, entry_call,strategy_period=1,strategy_cal=1,take_profit=0,total_profit=0,no_entry=0):
    balance = 0
    while True:
        try:
            if platform == 1:  # binance
            ...
            if in_position == False and gas > 20:
                ...
                if direction == 'Long' and balance >= first_amount:
                    ...
                    if in_position == False and strategy_period > 1 and strategy_period > strategy_cal:
            ...
            if in_position == True  and direction == 'Long':
                ...
                if current_profit > 0:
                    ...
                    if strategy_cal >= strategy_period:
                        start_bot = 0
...
def bot():
    ...
    if trade_type == "0":
        spotBot(user_id, setid, beforeqty, flag, sold, gas, first_buy, first_price, platform, api_key, api_secret, api_passphrase, in_position, buy_position, sell_position, coin, trade_price,
           firstbuy_amount, qty, double_position, margin_call, profit_ratio, whole_ratio, price_drop, m_ratio, profit_callback, buycallback, position_amount)
    elif trade_type == "1":
        featuresBot(user_id, setid, gas, first_buy, first_price, platform, api_key, api_secret, api_passphrase, in_position, buy_position, sell_position, coin, trade_price,
           firstbuy_amount, qty, price_drop, m_ratio, position_amount, leverage, direction, stop_loss, price_above, price_below, re_capital, closing_price, entry_call,profit_callback)
