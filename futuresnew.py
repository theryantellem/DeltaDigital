...
def Posttrade(setid=0, user_id=0, platform=0, qty=0, in_position=False, trade_price=0.00, tgmessage=
"0", first_buy=0.00, position_amount=0, first_price=0.00, balance=0.00, re_capital=0.00, total_profit=0, trade_type=1, current_profit=0):
...
"",first_buy=0,position_amount=0,balance=0,trade_type=1, percent=0):
        ...
        try:
            ...
            if in_position == False and gas > 10:
                ...
                if in_position == False and direction == 'Long' and balance >= first_amount:
                    ...
                    Posttrade(setid=setid, user_id=user_id, platform=platform,qty=quantity,in_position=in_position,trade_price=trade_price,tgmessage=tgmessage,first_buy=entry_call,position_amount=position_amount,first_price=first_price,balance=balance)
            ...
            if in_position == True and platform == 1:
                ...
                if rate >= 1000:
                    ...
                    Posttrade(setid=setid, user_id=user_id,platform=platform, qty=qty, in_position=in_position, trade_price=trade_price, tgmessage=details,
                              first_buy=entry_call,first_price=first_price, balance=balance)
            ...
            if in_position == True:
                ...
                if position_amount == None:
                    in_position = False
                    # print("postion none")
                    Posttrade(setid=setid, user_id=user_id, platform=platform,in_position=in_position,first_buy=entry_call,trade_price=trade_price, first_price=first_price,position_amount=position_amount)
                    return jsonify({"Status": "SUCCESS"})
                ...
                Posttrade(setid=setid, user_id=user_id, platform=platform,qty=qty,in_position=in_position,trade_price=trade_price,first_buy=entry_call,position_amount=position_amount,first_price=first_price,balance=balance)
                ...
                if current_profit >= expectprofit:
                    ...
                    if gas < 10:
                        ...
                        Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                                  current_profit=current_profit, qty=qty, position_amount=position_amount, tgmessage=details, first_buy=entry_call, trade_price=trade_price, first_price=first_price)

                    if profit >= 0.00001:
                        Settletrade(setid=setid, user_id=user_id, platform=platform, qty=qty, coin=coin, in_position=in_position, profit=profit,
                                     trade_price=trade_price, tgmessage="", first_buy=entry_call, position_amount=qtyusdt, balance=balance,
                                     percent=percent)

                    if profit < 0.00001:
                        Settletrade(setid=setid, user_id=user_id, platform=platform,qty=qty, coin=coin, in_position=in_position, profit=profit,
                                    trade_price=trade_price, tgmessage="",
                                    first_buy=entry_call, position_amount=qtyusdt, balance=balance,
                                    percent=percent)

                    Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                              current_profit=profit, qty=qty, position_amount=position_amount, first_buy=entry_call, first_price=first_price,trade_price=trade_price)
                ...
                # if current_profit > 0:
                    ...
                    if gas < 10:
                        ...
                        Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                                  ...
                                  tgmessage=details, first_buy=entry_call, trade_price=trade_price, first_price=first_price)
                    ...
                    Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                              current_profit=profit, qty=qty, position_amount=position_amount, first_buy=entry_call, trade_price=trade_price, first_price=first_price)
                    ...
                    if trade_price < cal2:
                        ...
                        if balance < tradec:
                            ...
                            Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                                      ...
                                      tgmessage=details, first_buy=entry_call,first_price=first_price,trade_price=trade_price)
                            return jsonify({"Status": "SUCCESS"})
                        ...
                        tgmessage= f"No {entry_call} martingale long succesfully filled. "

                        balance = GetBalance(exchange, platform)

                        Posttrade(setid=setid, user_id=user_id, platform=platform,tgmessage=tgmessage, in_position=in_position,
                                  current_profit=profit, qty=qty, position_amount=position_amount, first_buy=entry_call, first_price=first_price, trade_price=trade_price)
                        return jsonify({"Status": "SUCCESS"})
            ...
            if direction == 'Short':
                ...
                if in_position == True and entry_call !=  margin_call:
                    ...
                    if trade_price >= cal2:
                        qty = float(first_amount) * float(m_ratio[entry_call]) / float(trade_price)
                        ...
                        quantity = qty
                        entry_call = entry_call + 1
                        tgmessage= f"No {entry_call} martingale short succesfully filled."
                        
                        Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                                  current_profit=current_profit, qty=qty,balance=balance,
                                  position_amount=position_amount, trade_price=trade_price, first_buy=entry_call,
                                  tgmessage=tgmessage, first_price=first_price)
            ...
            if in_position == True:
                Posttrade(setid=setid, user_id=user_id, platform=platform, in_position=in_position,
                          current_profit=current_profit, qty=qty,
                          position_amount=position_amount,
                          first_buy=entry_call, first_price=first_price, trade_price=trade_price)
                return jsonify({"Status": "SUCCESS"})
