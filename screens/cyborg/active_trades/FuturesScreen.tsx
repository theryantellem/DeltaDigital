...
import {activeStrategy, binanceTicker, quantitativeStrategies} from "../../../api";
...
//   const {data:tickers}= useQuery(['binanceTicker'],binanceTicker)
    ...
    const renderHeader = useCallback(() => (
        <>
                    ...
                    <View style={styles.balanceGraph}>

                        {
                            ...
                            <Text
                                ...
                                {currencyFormatter('en-US', 'USD').format(Strategies?.data?.futures_binance_balance)}
                            </Text>
                        }
                        {
                            ...
                            <Text
                                ...
                                {currencyFormatter('en-US', 'USD').format(Strategies?.data?.futures_kucoin_balance)}
                            </Text>
                        }
                        {
                            ...
                            <Text
                                ...
                                {currencyFormatter('en-US', 'USD').format(Strategies?.data?.futures_coinbasepro_balance)}
                            </Text>
                        }

                        {
                            ...
                            <Text
                                ...
                                {currencyFormatter('en-US', 'USD').format(Strategies?.data?.futures_kraken_balance)}
            ...
            {
                data?.data['Operation Strategy']!== null && data?.data['Operation Strategy'].filter((market:{trade_type:string}) => market.trade_type == '1')
                    .filter((market:{exchange:string}) => market.exchange == tabExchange).length < 1
                &&

                <NoItem message={"No active trades on Spot"}/>
            }

            {
                data?.data['Operation Strategy'] == null &&
                <NoItem message={"No active trades on Spot"}/>
            }
    ...
    useRefreshOnFocus(refetchStrategies)
